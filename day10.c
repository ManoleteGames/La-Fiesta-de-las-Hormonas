...
void D10_GoToNextDay(int x, int y){
   ...
   LoadImage("IMAGES2.DAT","endday.pcx",2); // Load menu background image to non visible page
   ...
   LoadText("D10_STR.DAT","D10GLB.txt","070",string,&length);
	...
	switch(player.floor){
      ...
      case 2: // Floor 2
      	if(player.floor2_event_mask[event] == 1){
            ...
            if(player.event == 3){ // Ir al examen
